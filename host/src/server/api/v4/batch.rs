use crate::{
    ...
    server::{api::v3::Status, utils::to_v3_status},
};
...
async fn batch_handler(
    ...
    let mut sub_betch_ids = Vec::with_capacity(batch_request.batches.len());
    for BatchMetadata {
        batch_id,
    ...
    } in batch_request.batches.iter()
    {
        ...
        sub_betch_ids.push(*batch_id);
    }

    let agg_request_key =
        AggregationRequestKey::new(batch_request.proof_type, sub_betch_ids.clone());
