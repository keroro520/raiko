...
pub async fn prove(
    ...
    check_system_paused(&actor)?;
    debug_request_status(&actor, &request_key.clone().into());

    // Return if request is already completed successfully
    let status_opt = actor.pool_get_status(&request_key.clone().into())?;
...
pub async fn cancel_aggregation(
    ...
    for sub_request_key in sub_request_keys {
        let _ = cancel(actor, sub_request_key.into()).await?;
    }

    cancel(actor, request_key.into()).await
}

fn check_system_paused(actor: &Actor) -> Result<(), String> {
    if actor.is_paused() {
...
fn debug_request_status(actor: &Actor, request_key: &RequestKey) {
    if let Ok(status_opt) = actor.pool_get_status(request_key) {
